...
This is done by manually defining a symbol:

```{r}
reticulate::py_run_string("y = Symbol('y', positive = True)")
```

Note that the variable `y` exists in Python but not in `R`.
...
We can get a handle on `y` as follows:
