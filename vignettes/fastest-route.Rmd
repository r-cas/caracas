...
\[
  k = `r inline_code(tex(best_k))` \approx `r inline_code(round(as_r(best_k), 2))` .
\]
...
```
\[
  `r inline_code(tex(best_route))` \approx `r inline_code(round(as_r(best_route), 2))`
\]
...
```
\[
  `r inline_code(tex(best_l))` \approx `r inline_code(round(as_r(best_l), 2))`
\]
